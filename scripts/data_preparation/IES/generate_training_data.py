...
add_holiday = True  # Add holiday as a feature
steps_per_day = 24  # Number of time steps per day
frequency = 1440 // steps_per_day
domain = 'IES'
feature_description = [domain, 'time of day', 'day of week', 'month of year','holiday']
regular_settings = {
    'INPUT_LEN': 168,
    'OUTPUT_LEN': 24,
...
def add_temporal_features(data, df):
    ...
    if add_month_of_year:
        ...
        moy = (df.index.month - 1) / 12 # df.index.month starts from 1. We need to minus 1 to make it start from 0.
        moy_tiled = np.tile(moy, [1, n, 1]).transpose((2, 1, 0))
    ...
    if add_holiday:
        ...
        # 检查每个日期是否为假期
        holiday = df.index.map(lambda x: x in us_holidays).astype(np.float32)
        holiday_tiled = np.tile(holiday, [1, n, 1]).transpose((2, 1, 0))
        feature_list.append(holiday_tiled)
